...
def preprocessing(image_data, final_height, final_width, augmentation_fn=None, evaluate=False):
    ...
    image = tf.cast(tf.image.resize(image, (final_height, final_width)), tf.float32)
    ...
    if augmentation_fn:
        image, bboxes = augmentation_fn(image, bboxes)
...
#     return dataset, info
def get_dataset(image_dir, ann_file, tfrecord_file):
    return get_tfrecord_dataset(image_dir, ann_file, tfrecord_file)
